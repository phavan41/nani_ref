...
@Injectable()
export  class StopsWithGeofenceService {
    constructor(public http: HttpClient) { }
    
    getStopsWithGeofenceReportExport(data){
      let headers = new Headers();
      headers.append('Content-Type', 'application/json');
      let options = new RequestOptions({ headers: headers });
      return this.http.post(environment.serviceurl + '/admin/ReportsController/stopsWithGeofenceExport', JSON.stringify(data)).map((res: any) => res);
  }
    getStopsWithGeofenceReport(Paging:any) {
      return this.http.post(environment.serviceurl + '/admin/ReportsController/stopsWithGeofence', JSON.stringify(Paging)).map((res: any) => res);
      }
